...
_VALID_OPTION_TYPES = frozenset({"C", "P", "IC", "IP"})


def _validate_scalar(name: str, value: float, *, positive: bool = False) -> None:
    """Validate a finite scalar and, where required, its sign."""
...
def _validate_slice_array(
    ...
    *,
    ...
    """Validate one numeric quote array and return its NumPy view."""
...
def _validate_option_slice_data(strikes: np.ndarray, optiontypes: np.ndarray) -> int:
    """Validate the aligned strike and payoff-code vectors of one maturity."""
    strikes_array = np.asarray(strikes)
    optiontypes_array = np.asarray(optiontypes)
    if strikes_array.ndim != 1 or optiontypes_array.ndim != 1:
        raise ValueError("strikes and optiontypes must be one-dimensional")
    if strikes_array.size == 0:
        raise ValueError("strikes and optiontypes must not be empty")
    if strikes_array.size != optiontypes_array.size:
        raise ValueError("strikes and optiontypes must have the same length")
    if not np.all(np.isfinite(strikes_array)) or np.any(strikes_array <= 0.0):
        raise ValueError("strikes must contain only finite positive values")
    invalid = set(optiontypes_array.astype(str)) - _VALID_OPTION_TYPES
    if invalid:
        raise ValueError(f"unsupported optiontypes: {sorted(invalid)}")
    return strikes_array.size
...
class OptionSlice:
    ...
    def __post_init__(self):
        """
        validate aligned inputs and make discount conventions consistent.
        """
        _validate_scalar("ttm", self.ttm, positive=True)
        _validate_scalar("forward", self.forward, positive=True)
        size = _validate_option_slice_data(self.strikes, self.optiontypes)
        if self.discfactor is not None:
            _validate_scalar("discfactor", self.discfactor, positive=True)
            self.discount_rate = - np.log(self.discfactor) / self.ttm
        elif self.discount_rate is not None:
            _validate_scalar("discount_rate", self.discount_rate)
        ...
        arrays = (
            ("bid_ivs", self.bid_ivs, True, False),
            ("ask_ivs", self.ask_ivs, True, False),
            ("bid_prices", self.bid_prices, False, True),
            ("ask_prices", self.ask_prices, False, True),
        )
        validated = {}
        for name, values, positive, nonnegative in arrays:
            if values is not None:
                validated[name] = _validate_slice_array(
                    name, values, size, positive=positive, nonnegative=nonnegative
                )
        if self.bid_ivs is not None and self.ask_ivs is not None:
            if np.any(validated["bid_ivs"] > validated["ask_ivs"]):
                raise ValueError("bid_ivs must not exceed ask_ivs")
        if self.bid_prices is not None and self.ask_prices is not None:
            if np.any(validated["bid_prices"] > validated["ask_prices"]):
                raise ValueError("bid_prices must not exceed ask_prices")
...
class OptionChain:
    ...
    def __post_init__(self):
        """
        validate chain dimensions and make discount conventions consistent.
        """
        ttms = np.asarray(self.ttms)
        forwards = np.asarray(self.forwards)
        if ttms.ndim != 1 or ttms.size == 0:
            raise ValueError("ttms must be a non-empty one-dimensional array")
        if not np.all(np.isfinite(ttms)) or np.any(ttms <= 0.0):
            raise ValueError("ttms must contain only finite positive values")
        if np.any(np.diff(ttms) <= 0.0):
            raise ValueError("ttms must be strictly increasing")
        if forwards.ndim != 1 or forwards.size != ttms.size:
            raise ValueError("ttms and forwards must have the same one-dimensional length")
        if not np.all(np.isfinite(forwards)) or np.any(forwards <= 0.0):
            raise ValueError("forwards must contain only finite positive values")
        if (
            len(self.strikes_ttms) != ttms.size
            or len(self.optiontypes_ttms) != ttms.size
        ):
            raise ValueError("ttms, strikes_ttms, and optiontypes_ttms must have the same length")
        if self.ids is not None and len(self.ids) != ttms.size:
            raise ValueError("ids and ttms must have the same length")

        if self.discfactors is not None:
            _validate_slice_array(
                "discfactors", self.discfactors, ttms.size, positive=True
            )
            self.discount_rates = - np.log(self.discfactors) / self.ttms
        elif self.discount_rates is not None:
            _validate_slice_array("discount_rates", self.discount_rates, ttms.size)
        ...
        if self.forwards0 is not None:
            _validate_slice_array("forwards0", self.forwards0, ttms.size, positive=True)
        ...
        for name, values, _, _ in quote_lists:
            if values is not None and len(values) != ttms.size:
                raise ValueError(f"{name} and ttms must have the same length")

        for idx, (strikes, optiontypes) in enumerate(
            zip(self.strikes_ttms, self.optiontypes_ttms)
        ):
            size = _validate_option_slice_data(strikes, optiontypes)
            validated = {}
            for name, values, positive, nonnegative in quote_lists:
                if values is not None:
                    validated[name] = _validate_slice_array(
                        f"{name}[{idx}]",
                        values[idx],
            ...
            if self.bid_ivs is not None and self.ask_ivs is not None:
                if np.any(validated["bid_ivs"] > validated["ask_ivs"]):
                    raise ValueError(f"bid_ivs[{idx}] must not exceed ask_ivs[{idx}]")
            if self.bid_prices is not None and self.ask_prices is not None:
                if np.any(validated["bid_prices"] > validated["ask_prices"]):
                    raise ValueError(f"bid_prices[{idx}] must not exceed ask_prices[{idx}]")
    ...
    def get_uniform_chain(cls,
        ...
        Used to plot model implied volatilities where no market data is needed. A flat forward
        vector is broadcast to the requested maturities, which keeps the default unit forward
        useful when callers provide a custom maturity grid.
        """
        ttms = np.asarray(ttms, dtype=float)
        forwards = np.asarray(forwards, dtype=float)
        if (
            forwards.ndim == 1
            and forwards.size != ttms.size
            and forwards.size > 0
            and np.all(forwards == forwards[0])
        ):
            forwards = np.full(ttms.size, forwards[0], dtype=float)
