...
def compute_integration_weights(var_grid: np.ndarray,
    ...
    variable of integration in the valuation formulas. Both supported composite
    rules require a finite, strictly increasing, uniformly spaced grid. Simpson
    additionally requires an odd number of points.
    ...
    ------
    ValueError
        If the grid is too short, non-finite, non-uniform, not increasing, or
        has an even number of points for Simpson's rule.
    """
    p = np.imag(var_grid)
    min_size = 3 if is_simpson else 2
    if len(p) < min_size:
        raise ValueError("integration grid is too short for the selected rule")
    if not np.all(np.isfinite(p)):
        raise ValueError("integration grid must contain only finite values")
    steps = p[1:] - p[:-1]
    if np.any(steps <= 0.0):
        raise ValueError("integration grid must be strictly increasing")
    spacing_tolerance = 1.0e-12 * max(1.0, np.abs(steps[0]))
    if np.any(np.abs(steps - steps[0]) > spacing_tolerance):
        raise ValueError("integration grid must be uniformly spaced")

    if is_simpson:
        if len(p) % 2 == 0:
            raise ValueError("Simpson integration requires an odd number of grid points")
...
@njit(cache=False, fastmath=True)
def _compute_legacy_pricer_weights(
    var_grid: np.ndarray, is_simpson: bool = True
) -> np.ndarray:
    """Return the historical Fourier-pricer weights without changing baselines."""
    p = np.imag(var_grid)
    if is_simpson:
        dp = 2.0 * np.ones(len(p))
        dp[0] = 1.0
        ...
        for i in range(len(p)):
            if i % 2 == 1:
                dp[i] = 4.0
        return ((p[1] - p[0]) / 3.0) * dp
    return np.append(0.5 * (p[1] - p[0]), p[1:] - p[:-1])
...
def vanilla_slice_pricer_with_mgf_grid(log_mgf_grid: np.ndarray,
    ...
    """
    p = np.imag(phi_grid)
    dp = _compute_legacy_pricer_weights(var_grid=phi_grid, is_simpson=is_simpson)
...
def slice_qvar_pricer_with_a_grid(log_mgf_grid: np.ndarray,
    ...
    log_strikes = np.log(forward / strikes)
    dp = _compute_legacy_pricer_weights(var_grid=psi_grid, is_simpson=is_simpson)
...
def pdf_with_mgf_grid(log_mgf_grid: np.ndarray,
    ...
    grid can be non-uniform
    """
    dp = _compute_legacy_pricer_weights(
        var_grid=transform_var_grid, is_simpson=is_simpson
    ) / np.pi
