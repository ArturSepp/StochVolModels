...
class CalibrationError(RuntimeError):
    """Raised when an optimizer cannot produce a usable calibration result."""


def validate_optimization_result(result, bounds) -> np.ndarray:
    """Return a finite, in-bounds optimizer vector or raise ``CalibrationError``."""
    message = str(getattr(result, "message", "no optimizer message"))
    if not bool(getattr(result, "success", False)):
        raise CalibrationError(f"Calibration failed: {message}")

    raw_values = getattr(result, "x", None)
    if raw_values is None:
        raise CalibrationError(f"Calibration returned no parameter vector: {message}")
    try:
        values = np.asarray(raw_values, dtype=float)
    except (TypeError, ValueError) as error:
        raise CalibrationError(
            f"Calibration returned a non-numeric parameter vector: {message}"
        ) from error
    if values.ndim != 1 or values.size != len(bounds):
        raise CalibrationError(
            f"Calibration returned a parameter vector with the wrong shape: {message}"
        )
    if not np.all(np.isfinite(values)):
        raise CalibrationError(f"Calibration returned non-finite parameters: {message}")

    tolerance = 1.0e-10
    for value, (lower, upper) in zip(values, bounds):
        if lower is not None and value < lower - tolerance:
            raise CalibrationError(
                f"Calibration returned parameters below bounds: {message}"
            )
        if upper is not None and value > upper + tolerance:
            raise CalibrationError(
                f"Calibration returned parameters above bounds: {message}"
            )
    return values
