...
def Gaussian_interval(H, m, a, b, fractional_weight=True):
    ...
    """
    raise ImportError(
        "Gaussian_interval is an experimental compatibility function that requires "
        "the unsupported optional packages orthopy and quadpy"
    )
...
class kernel_rheston:
    ...
    This compatibility class remains experimental because the original implementation
    did not provide or declare a Mittag-Leffler evaluator.
    """
    ...
    def _k(self, r):
        """Fail precisely until a characterized Mittag-Leffler backend is supported."""
        raise NotImplementedError(
            "kernel_rheston is experimental: no characterized Mittag-Leffler "
            "evaluator is currently supported"
        )
