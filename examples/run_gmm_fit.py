...
# selct two maturities
option_chain = OptionChain.get_slices_as_chain(option_chain=option_chain, ids=['2m', '6m'])
...
for idx, (key, params) in enumerate(fit_params.items()):
    ...
    plot_gmm_pdfs(params=params, option_chain0=option_chain0, axs=axs[idx, :])
    qis.set_title(ax=axs[idx, 0], title=f"{key}-slice: (A) State PDF and Aggregate Risk-Neutral PDF")
    qis.set_title(ax=axs[idx, 1], title=f"{key}-slice: Model to Market Bid/Ask vols")
