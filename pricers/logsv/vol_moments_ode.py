...
def compute_analytic_qvar(params: LogSvParams,
    ...
    if np.isclose(t, 0.0):
        ...
        int_moments = compute_analytic_vol_moments(params=params, t=t, n_terms=n_terms, is_qvar=True)
        qvar = (int_moments[1] + 2.0*params.theta*int_moments[0]) / t + params.theta2
    return qvar
...
def compute_vol_moments_t(params: LogSvParams,
    ...
    moments = np.zeros((len(t), n_terms))
...
def compute_sqrt_qvar_t(params: LogSvParams, t: np.ndarray, n_terms: int = 4) -> np.ndarray:
    ev = np.zeros(len(t))
    for idx, t_ in enumerate(t):
        ev[idx] = np.sqrt(compute_analytic_qvar(t=t_, params=params, n_terms=n_terms))
    return ev
...
def run_unit_test(unit_test: UnitTests):
    ...
    if unit_test == UnitTests.VOL_MOMENTS:
        ...
        for n in np.arange(n_terms):
            ...
            mcs.append(mc)
        analytic_vol_moments = compute_vol_moments_t(params=params, t=grid_t, n_terms=n_terms)
