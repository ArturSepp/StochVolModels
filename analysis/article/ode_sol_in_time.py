...
def run_unit_test(unit_test: UnitTests):
    ...
    elif unit_test == UnitTests.DETECT_EXPLOSION:
        params_list = []
        betas = np.linspace(0.5*params.beta, 4.0*params.beta, 10)
        volvols = np.linspace(0.5*params.volvol, 4.0*params.volvol, 20)
        is_explosion = np.full((betas.size, volvols.size), False)
        pos_eigenvals = np.full((betas.size, volvols.size), True)
        idx_phi = 0
        for i_beta, beta in enumerate(betas):
            for j_volvol, volvol in enumerate(volvols):
                params_i = LogSvParams(sigma0=params.sigma0, theta=params.theta, kappa1=params.kappa1, kappa2=params.kappa2,
                                       beta=beta, volvol=volvol)
                is_explosion_, pos_eigenvals_ = detect_ode_explosion(params=params_i, ttm=ttm,
                                                                          expansion_order=ExpansionOrder.FIRST)
                is_explosion[i_beta, j_volvol] = is_explosion_[idx_phi]
                pos_eigenvals[i_beta, j_volvol] = pos_eigenvals_[idx_phi]

        is_explosion = pd.DataFrame(is_explosion, columns=volvols, index=betas)
        pos_eigenvals = pd.DataFrame(pos_eigenvals, columns=volvols, index=betas)
        # np.savetxt('c:/temp/is_explosion.csv', is_explosion, delimiter=',')
        # np.savetxt('c:/temp/pos_eigenvals.csv', pos_eigenvals, delimiter=',')
        is_explosion.to_csv("c:/temp/is_explosion.csv")
        pos_eigenvals.to_csv("c:/temp/pos_eigenvals.csv")
