"""
Interest rate conventions: discount factors, bonds, swap and Libor rates.

Leaf module with no intra-package imports, consumed by the factor HJM pricers and
by the swaption chain container in ``stochvolmodels.data.option_chain``. Discount
factors come from :func:`df_fast`, a hardcoded flat-curve stub, so nothing here
reads market data.
"""
import numpy as np
...
def bracket(ts: np.ndarray, t: float, throw_if_not_found: bool = False) -> int:
    """
    index of the first element of ``ts`` at or above ``t``, or -1 if none.

    Raises ValueError instead of returning -1 when ``throw_if_not_found`` is set.
    """
    # if 0 in ts:
...
def pw_const(ts: np.ndarray,
    ...
    """
    piecewise constant interpolation of ``vs`` on the knots ``ts``, evaluated at t.

    Parameters
    ----------
    ts : np.ndarray
        Knot abscissae, ascending.
    vs : np.ndarray
        Values on the knots.
    t : float
        Evaluation point.
    flat_extrapol : bool, default False
        Hold the last value beyond the final knot rather than failing.
    shift : int, default 0
        Offset applied to the located index, for staggered value grids.
    """
...
def get_default_swap_term_structure(expiry: float, tenor: float):
    """annual payment dates of a swap starting at ``expiry`` and running ``tenor`` years."""
    freq = 1.0
...
def get_futures_start_and_pmt(t0: float, lag: float,
    ...
    """accrual start and end of the Libor period underlying a futures contract."""
    start = t0 + lag
...
def df_fast(t: Union[float, np.ndarray], ccy: str = "USD") -> float:
    """
    discount factor from a hardcoded flat zero rate.

    A stub, not a curve: the rate is fixed per currency in the body. Any pricing
    that must respect a market curve should not route through here.
    """
...
def bond_grad(bond_value, B_PX):
    """gradient of a bond price with respect to the state variables, dB/dx_i = B b_i."""
...
def swap_grad(numer0: np.ndarray,
    ...
    """
    quotient rule for the swap rate gradient: d(N/D) = dN/D - N dD / D^2.

    Handles the scalar and the vector-of-states cases; mixed dimensions fall
    through to the broadcasting branch.
    """
...
def generate_ttms_grid(ttms: np.ndarray,
    ...
    """union of uniform sub-grids spanning consecutive maturities, starting at zero."""
    t0 = 0
    t_grid = np.array([0])
...
def to_yearfrac(d1, d2):
    """year fraction between two dates already expressed in years, that is d2 - d1."""
    return d2 - d1
#######################################################


def divide_mc(arr2d, arr1d):
    """divide each column of a (path, state) array by a per-path vector."""
...
def prod_mc(arr2d, arr1d):
    """multiply each column of a (path, state) array by a per-path vector."""
    assert arr2d.ndim == 2 and arr1d.ndim == 1
    res = np.zeros_like(arr2d)
    for j, ann_derj in enumerate(arr2d.T):
        res[:, j] = ann_derj * arr1d
...
def swap_rate(ccy: str,
    ...
    """
    par swap rate for the payment schedule ``ts_sw`` seen at time ``t``.

    Ratio of the floating leg value to the annuity, both discounted with
    :func:`df_fast` and normalized by the discount factor at t.
    """
    denumer0 = 0
...
def libor_rate(ccy: str,
               t: float, tenor: float):
    """simply compounded forward rate over [t, t + tenor] implied by :func:`df_fast`."""
...
@njit(cache=False, fastmath=True)
def G(k, t, T):
    """Hull-White mean-reversion factor G(t, T) = (1 - exp(-k (T - t))) / k."""
