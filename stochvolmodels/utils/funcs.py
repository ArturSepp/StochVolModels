...
def set_time_grid(ttm: float, nb_steps_per_year: int = 360) -> Tuple[int, float, np.ndarray]:
    ...
    set daily steps
    """
    dt = 1.0 / nb_steps_per_year
    grid_t = np.arange(0.0, ttm+1e-6, dt)
    nb_steps = int(ttm * nb_steps_per_year)
...
def erfcc(x: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    ...
    t = 1. / (1. + 0.5 * z)
    r = t * np.exp(
        -z * z - 1.26551223 + t * (1.00002368 + t * (0.37409196 + t * (0.09678418 + t * (-0.18628806 + t * (0.27886807 +
                                                                                                            t * (
                                                                                                                        -1.13520398 + t * (
                                                                                                                            1.48851587 + t * (
                                                                                                                                -.82215223 + t * 0.17087277)))))))))
...
@njit(cache=False, fastmath=False)
def pos(vec: np.ndarray) -> np.ndarray:
